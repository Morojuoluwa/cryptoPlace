   ...
   <div className=' px-[10px] pb-[100px]'>
    <div className=' max-w-[600px] my-[40px] md:my-[80px] gap-y-3 mx-auto flex flex-col items-center text-center'>
        ...
        <form onSubmit={searchHandler} className='p-2 w-[70%] md:w-[80%] bg-white rounded-[5px] text-[12px] md:text-[20px] flex justify-between items-center gap-[10px]'>
            <input onChange={inputHandler} className='flex flex-1 outline-none border-none pl-[4px] md:pl-[10px]' type='text' placeholder='search crypto' value={input} required/>
    ...
    <div className=' max-w-[800px] m-auto bg-crypto-bg rounded-[15px]'>
        ...
        
        {
            displayCoin.slice(0,10).map((item, index)=>(
                <div key={index} className='max-sm:text-[12px] last:border-none grid-cols-[0.5fr_2fr_1fr_1fr] md:grid-cols-[0.5fr_2fr_1fr_1fr_1.5fr] grid text-white  py-[15px] px-5 border-b-[1px] border-solid border-b-[#3c3c3c]'>
...
{/* grid-cols-[0.5fr_3fr_1fr-1fr] md:grid-cols-[0.5fr_2fr_1fr_1fr_1.5fr] */}
    </div>
